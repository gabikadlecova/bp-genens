...
\ref{alg:ourgrow}). The tree is grown from root, which
...
\caption{Modified grow method for individual initialization\label{alg:ourgrow}}
  ...
  $S_{func}$ and $S_{term}$, hyperparameter dictionary $H$}
  \KwResult{generated individual}
  \SetKwFunction{Arity}{choose\_arity}
  \SetKwFunction{HParams}{choose\_hyperparams}
  \;
  $h \longleftarrow$ randomly select tree height from interval $[1, max\_height]$ 
  \;  
  \If{$h > 1$}{
     set $cPipe$ or $cPred$ as the root of $ind$
  }  
  \Else{
      \KwRet{a random classifier}
  }
  
  \;
  \While{$ind$ has a function with a child missing}{
      ...
      add the child $node$ as an argument of $f$
  }
  \KwRet{$ind$}
...
actual model combination method remains the same. Therefore, it is represented
by a single nod. This means
that the actual arity is determined just when the node is about to be inserted
into the tree. The arity is determined by an interval which may or may not have
...
The list has been created by slightly extending the hyp. list composed by \ref{aaa}. 
