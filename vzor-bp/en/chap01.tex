...
% Goals - what, how, why
There seems to be no general definition of AutoML as for now. Informally,
AutoML encompasses methods that automatize a part of a machine learning
workflow. Examples of existing methods are presented in chapter
\ref{ch2:related}.

\subsection{Machine learning workflows}
\textit{Here will be a pretty workflow image} % compliant to PDF/A
...
pipeline'. However, in the context of AutoML it may not be a suitable term,
as a pipeline is a linear acyclic graph and some parts of the flow may be
in fact iterative, as can be seen in figure ???.
% TODO
